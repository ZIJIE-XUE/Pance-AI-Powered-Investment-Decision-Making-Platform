...
def _render_header():
    ...
    st.markdown(
        '<div class="temp-hero">'
        f'<h1>{t("🌡️ 智能温度定投")}</h1>'
        f'<p>{t("基于市场估值温度动态调整每期定投金额 · 市场冷时多投，热时少投 · 回测验证，数据驱动决策")}</p>'
        '</div>',
        unsafe_allow_html=True,
    )
...
def _render_config() -> dict:
    ...
    with col2:
        years = st.selectbox(
            ...
            options=[3, 5, 7, 10],
            index=1,
            help=t("回测历史年数。年限越长，穿越的牛熊周期越多，结论越可靠"),
        )
    ...
    with col3:
        base_monthly = st.number_input(
            ...
            step=500,
            format="%d",
            help=t("温度适中(40-60°C)时的标准月投金额。市场冷热时按倍率自动调整"),
        )
    ...
    with col4:
        strategy_name = st.radio(
            ...
            index=0,
            help=t("积极=低温重仓高位空仓 · 适中=均衡加减 · 保守=温和微调"),
        )
...
def _render_multiplier_map(strategy_name: str):
    ...
    st.markdown(
        ...
        f"{t('📐 温度→倍率映射 · ')}{strategy['name']}{t('型 · ')}{strategy['desc']}</p>",
        unsafe_allow_html=True,
    )

    cols = st.columns(5, gap="small")
    zones = [
        (t("🧊 极度低估<br>0–20°C"), "#2196F3"),
        (t("❄️ 偏低<br>20–40°C"), "#64B5F6"),
        (t("🌡️ 适中<br>40–60°C"), "#9E9E9E"),
        (t("🔥 偏贵<br>60–80°C"), "#FF9800"),
        (t("💥 高估<br>80–100°C"), "#f44336"),
    ]
...
def _render_equity_chart(backtest: dict, config: dict):
    ...
    fig.add_trace(go.Scatter(
        ...
        name=t("🌡️ 温度定投"),
        hovertemplate=t("%{x|%Y-%m}<br>温度定投: ¥%{y:,.0f}<extra></extra>"),
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        name=t("📋 普通定投"),
        hovertemplate=t("%{x|%Y-%m}<br>普通定投: ¥%{y:,.0f}<extra></extra>"),
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        name=t("💰 一次性买入"),
        hovertemplate=t("%{x|%Y-%m}<br>一次性: ¥%{y:,.0f}<extra></extra>"),
    ))
    ...
    if temp_records:
        fig.add_trace(go.Scatter(
            ...
            name=t("累计投入"),
            hovertemplate=t("%{x|%Y-%m}<br>累计投入: ¥%{y:,.0f}<extra></extra>"),
        ))
    ...
    if monthly_df is not None and len(monthly_df) > 0:
        ...
        fig.add_trace(go.Scatter(
            ...
            yaxis="y2",
            name=t("市场温度"),
            hovertemplate=t("%{x|%Y-%m}<br>温度: %{y:.0f}°C<extra></extra>"),
        ))
...
def _render_metrics(backtest: dict):
    ...
    for col, m, label, icon in [
        (col_t, temp_m, t("🌡️ 温度定投"), ORANGE_MAIN),
        (col_r, reg_m, t("📋 普通定投"), "#78909C"),
        (col_l, lump_m, t("💰 一次性买入"), "#42A5F5"),
    ]:
        with col:
            border_color = icon
            st.markdown(
    ...
    if cold or hot:
        ...
        with mech_cols[0]:
            ...
            st.markdown(
                ...
                f"<div style='font-size:0.85em;font-weight:600'>{t('🧊 低温区间 (0-40°C)')}</div>"
                f"<div style='font-size:0.75em;color:#888'>{t('共 {cold_months} 个月 · 估值偏低').format(cold_months=cold_months)}</div>"
                f"<div style='font-size:1.15em;font-weight:600;color:#1565C0;margin-top:4px'>"
                f"{'+' if cold_extra >= 0 else ''}¥{cold_extra:,.0f}</div>"
                f"<div style='font-size:0.72em;color:#555'>{t('额外加仓（低位多买）')}</div>"
                f"</div>",
                unsafe_allow_html=True,
            )
        ...
        with mech_cols[1]:
            ...
            st.markdown(
                ...
                f"<div style='font-size:0.85em;font-weight:600'>{t('🔥 高温区间 (60-100°C)')}</div>"
                f"<div style='font-size:0.75em;color:#888'>{t('共 {hot_months} 个月 · 估值偏高').format(hot_months=hot_months)}</div>"
                f"<div style='font-size:1.15em;font-weight:600;color:#E65100;margin-top:4px'>"
                f"¥{hot_saved:,.0f}</div>"
                f"<div style='font-size:0.72em;color:#555'>{t('暂扣少投（高位避风险）')}</div>"
                f"</div>",
                unsafe_allow_html=True,
            )
        ...
        with mech_cols[2]:
            ...
            st.markdown(
                ...
                f"<div style='font-size:0.85em;font-weight:600'>{t('💰 现金池动态')}</div>"
                f"<div style='font-size:0.75em;color:#888'>{t('峰值日期 {peak_d}').format(peak_d=peak_d)}</div>"
                f"<div style='font-size:1.15em;font-weight:600;color:#555;margin-top:4px'>"
                f"¥{max_cp:,.0f}</div>"
                f"<div style='font-size:0.72em;color:#555'>{t('最高积存金额')}</div>"
                f"</div>",
                unsafe_allow_html=True,
            )
...
def _render_yearly_breakdown(backtest: dict, config: dict):
    ...
    st.markdown("---")
    st.markdown(t("### 📋 温度定投逐年明细"))

    yearly = backtest.get("yearly_breakdown", [])
    if not yearly:
        st.info(t("数据不足以生成逐年明细"))
        return
    ...
    for y in yearly:
        ...
        table_data.append({
            t("年份"): t("{x}年").format(x=y['year']),
            t("当年投入"): f"¥{y['invested']:,.0f}",
            t("年末市值"): f"¥{y['end_value']:,.0f}",
            t("平均温度"): f"{avg_temp:.0f}°C {temp_info['zone_label']}",
            t("平均倍率"): f"×{y['avg_multiplier']:.2f}",
        })
    ...
    st.dataframe(
        ...
        column_config={
            ...
            t("平均温度"): st.column_config.TextColumn(width="medium"),
        },
    )
...
def _render_signal_validation(config: dict):
    ...
    """
    st.markdown("---")
    st.markdown(t("### 🔬 信号验证：温度能否预测未来收益？"))

    st.markdown(
        f'<p style="font-size:0.85em;color:#888;margin-bottom:12px">'
        f'{t("如果温度信号有效，低温应该对应未来较高的收益（负相关）。这是整个温度定投策略的实证基础。")}</p>',
        unsafe_allow_html=True,
    )

    with st.spinner(t("正在运行信号验证...（首次约 5-10 秒）")):
        validation = _fetch_signal_validation(config["index_name"])

    if "error" in validation:
        st.warning(t("⚠️ 信号验证暂不可用：{error}").format(error=validation['error']))
        return
    ...
    with chart_col:
        ...
        fig.add_trace(go.Scatter(
            ...
            name=t("月度数据点"),
            hovertemplate=t("温度: %{x:.0f}°C<br>未来12月收益: %{y:.1f}%<extra></extra>"),
        ))
        ...
        fig.add_trace(go.Scatter(
            ...
            name=t("回归线 (r={r:.3f})").format(r=corr['pearson_r']),
            hovertemplate=t("温度: %{x:.0f}°C<br>预测收益: %{y:.1f}%<extra></extra>"),
        ))
        ...
        fig.update_layout(
            height=380,
            margin=dict(l=20, r=20, t=10, b=20),
            xaxis_title=t("市场温度 (°C)"),
            yaxis_title=t("未来12个月收益 (%)"),
            plot_bgcolor="white",
            showlegend=True,
    ...
    with stat_col:
        ...
        sig_badge = t("✅ 统计显著") if corr["significant"] else t("⚠️ 不显著")
        ...
        st.markdown(
            ...
            f"<div style='font-size:0.85em;font-weight:600;margin-bottom:10px'>{t('📊 相关分析')}</div>"
            ...
            f"<tr><td>{t('p 值')}</td><td style='text-align:right;font-weight:700'>{corr['p_value']:.4f}</td></tr>"
            f"<tr><td>{t('R²')}</td><td style='text-align:right;font-weight:700'>{corr['r_squared']:.4f}</td></tr>"
            f"<tr><td>{t('显著性')}</td><td style='text-align:right;font-weight:700;color:{sig_color}'>{sig_badge}</td></tr>"
            f"<tr><td>{t('样本量')}</td><td style='text-align:right'>{t('{n} 个月').format(n=dq['total_points'])}</td></tr>"
            f"<tr><td>{t('基准指数')}</td><td style='text-align:right'>{dq['index_name']}</td></tr>"
            f"</table></div>",
            unsafe_allow_html=True,
        )
        ...
        for i, b in enumerate(buckets):
            with bucket_cols[i]:
                ...
                st.markdown(
                    ...
                    f"<div style='font-size:0.7em;color:#888'>{t('平均未来12月收益')}</div>"
                    f"<div style='font-size:0.7em;color:#aaa'>"
                    f"{t('正收益率 {rate}% · n={n}').format(rate=b['positive_rate'], n=b['count'])}</div>"
                    f"</div>",
                    unsafe_allow_html=True,
                )
...
def _render_regime_decomposition(backtest: dict):
    ...
    st.markdown("---")
    st.markdown(t("### 📊 市场环境分解：什么情况下温度定投有效？"))
    ...
    if not regimes:
        st.info(t("市场环境数据不足以进行分解分析"))
        return

    st.markdown(
        f'<p style="font-size:0.85em;color:#888;margin-bottom:12px">'
        f'{t("将回测区间拆分为牛市（价>均线+上行）、熊市（价<均线+下行）、震荡市（价在均线附近），对比三种策略在各环境下的表现。不同策略在不同市场中各有优劣——了解这一点是制定投资策略的关键。")}</p>',
        unsafe_allow_html=True,
    )
        ...
        with cols[i]:
            if data is None:
                st.markdown(
                    ...
                    f"<div style='font-size:0.8em;margin-top:8px'>{t('本期无此环境')}</div>"
                    f"</div>",
                    unsafe_allow_html=True,
                )
                continue
            ...
            strategies_html = ""
            for label, key in [(t("🌡️ 温度"), "temp_dca"), (t("📋 普通"), "regular_dca"), (t("💰 一次性"), "lump_sum")]:
                s = data[key]
                ret = s["return_pct"]
            ...
            st.markdown(
                ...
                f"{t('{months} 个月 · 平均温度 {avg_temp:.0f}°C').format(months=months, avg_temp=avg_temp)}</div>"
                f"{strategies_html}"
                f"</div>",
                unsafe_allow_html=True,
            )
    ...
    # ── Key takeaway ───────────────────────────────────────────────────────
    st.markdown(
        f'<div style="font-size:0.82em;color:#888;margin-top:4px">'
        f'{t("💡 <b>核心认知</b>：没有任何策略在所有市场中都最优。温度定投的价值定位是——用牛市中的相对落后，换取熊市和震荡市中的显著优势。这是一个明确、可预期的 trade-off，而非策略缺陷。")}'
        '</div>',
        unsafe_allow_html=True,
    )
...
def _render_walk_forward(config: dict):
    ...
    """
    st.markdown("---")
    st.markdown(t("### 🚶 Walk-Forward 样本外检验"))

    st.markdown(
        f'<p style="font-size:0.85em;color:#888;margin-bottom:12px">'
        f'{t("将历史数据拆分为训练窗（优化参数）和测试窗（只看一遍），测试策略是否在从未见过的数据上仍然有效。这是区分\"真正有用的策略\"和\"回测过拟合\"的黄金标准。")}</p>',
        unsafe_allow_html=True,
    )
    ...
    if test_years < 1:
        st.info(t("回测年限太短，无法进行有意义的 Walk-Forward 检验（至少需要2年训练 + 1年测试）。请增加回测年限。"))
        return

    with st.spinner(t("正在运行 Walk-Forward 检验...（{train_years}年训练 → {test_years}年测试，首次约 20-30 秒）").format(train_years=train_years, test_years=test_years)):
        wf = _fetch_walk_forward(config["index_name"], train_years, test_years)

    if "error" in wf:
        st.warning(t("⚠️ Walk-Forward 检验暂不可用：{error}").format(error=wf['error']))
        return
    ...
    with tw_col:
        st.markdown(
            ...
            f"{t('🔧 训练窗 · ')}{train_years}{t('年')}</div>"
            f"<div style='font-size:0.7em;color:#888;margin-bottom:10px'>"
            f"{dq['train_window']['start']} → {dq['train_window']['end']} · "
            f"{dq['train_months']} {t('个月')}</div>"
            f"<div style='font-size:0.78em;color:#555;margin-bottom:10px'>"
            f"{t('网格搜索 {n} 种组合，最优：PE {pw:.0%} + {label}').format(n=opt['search_space']['total_combos'], pw=opt['best_pe_weight'], label=opt['best_strategy_label'])}</div>"
            f"<div style='font-size:1.15em;font-weight:700'>"
            f"{t('温度')}: {train['temp_dca']['cagr_pct']}%</div>"
            f"<div style='font-size:0.8em;color:#888'>"
            f"{t('普通')}: {train['regular_dca']['cagr_pct']}% · "
            f"{t('超额')}: <b style='color:#E65100'>{'+' if train['excess_cagr'] > 0 else ''}{train['excess_cagr']}%</b></div>"
            f"<div style='font-size:0.8em;color:#888'>"
            f"{t('夏普')}: {train['temp_dca']['sharpe_ratio']}</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
    ...
    with ts_col:
        ...
        st.markdown(
            ...
            f"{t('👁️ 测试窗 · ')}{test_years}{t('年')}{t('（样本外）')}</div>"
            f"<div style='font-size:0.7em;color:#888;margin-bottom:10px'>"
            f"{dq['test_window']['start']} → {dq['test_window']['end']} · "
            f"{dq['test_months']} {t('个月 · 参数锁定，未接触')}</div>"
            f"<div style='font-size:0.78em;color:#555;margin-bottom:10px'>"
            f"PE {opt['best_pe_weight']:.0%} · {opt['best_strategy_label']} · {t('只看一遍')}</div>"
            f"<div style='font-size:1.15em;font-weight:700'>"
            f"{t('温度')}: {test['temp_dca']['cagr_pct']}%</div>"
            f"<div style='font-size:0.8em;color:#888'>"
            f"{t('普通')}: {test['regular_dca']['cagr_pct']}% · "
            f"{t('超额')}: <b style='color:{excess_color}'>{'+' if test['excess_cagr'] > 0 else ''}{test['excess_cagr']}%</b></div>"
            f"<div style='font-size:0.8em;color:#888'>"
            f"{t('夏普')}: {test['temp_dca']['sharpe_ratio']} "
            f"({t('变动')} {'+' if deg['sharpe_drop'] >= 0 else ''}{deg['sharpe_drop']})</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        hovertemplate=t("PE权重=%{x}<br>策略=%{y}<br>夏普=%{z:.2f}<extra></extra>"),
    ))
    ...
    st.markdown(
        ...
        f'{t("💡 <b>方法论</b>：Walk-Forward 是量化策略验证的黄金标准。训练窗内可自由探索参数，测试窗只看一次。若样本外仍然有效，则策略不是过拟合的产物——这正是学术论文和机构级策略验收的核心方法。")}'
        '</div>',
        unsafe_allow_html=True,
    )
...
def _render_behavioral_finance():
    ...
    """
    st.markdown("---")
    st.markdown(t("### 🧠 行为金融视角：为什么温度定投理论上应该有效？"))

    st.markdown(
        f'<p style="font-size:0.85em;color:#888;margin-bottom:16px">'
        f'{t("温度定投的价值不在于\"预测市场\"——没有人能准确预测。它的价值在于帮助投资者克服特定认知偏差，用规则取代情绪驱动决策。")}</p>',
        unsafe_allow_html=True,
    )
    ...
    biases = [
        {
            "emoji": "🧊",
            "zone": t("低估区间 (0-40°C)"),
            "bias": t("损失厌恶 (Loss Aversion)"),
            "bias_en": "Loss Aversion",
            "mechanism": t("市场恐慌时，投资者因害怕继续亏损而不敢买入。温度定投通过**自动加仓到 1.25–2.0 倍**，用规则强制逆情绪操作，克服'该买不敢买'的心理障碍。"),
            "color": "#2196F3",
        },
        {
            "emoji": "🔥",
            "zone": t("高估区间 (60-100°C)"),
            "bias": t("羊群效应 (Herd Behavior)"),
            "bias_en": "Herd Behavior",
            "mechanism": t("市场狂热时，投资者容易追涨杀跌。温度定投通过**强制少投至 0–0.75 倍**，在高位自动刹车，避免'别人都在买所以我也买'的从众陷阱。"),
            "color": "#FF9800",
        },
        {
            "emoji": "🌡️",
            "zone": t("适中区间 (40-60°C)"),
            "bias": t("锚定效应 (Anchoring)"),
            "bias_en": "Anchoring",
            "mechanism": t("投资者容易被近期价格'锚定'，在正常波动中过度反应。温度定投在估值合理时**维持基准定投**，不被短期价格波动所锚定，保持纪律性投资节奏。"),
            "color": "#9E9E9E",
        },
    ]
    ...
    # Summary
    st.markdown(
        f'<div style="margin-top:12px;padding:14px 18px;background:#E8EAF6;'
        f'border-radius:10px;font-size:0.88em;color:#283593;line-height:1.7">'
        f'{t("💡 <b>学术定位</b>：温度定投本质上是一种<b>规则化的行为金融干预工具</b>。它不依赖市场预测能力，而是通过系统性地抵消投资者的认知偏差来创造价值。这一视角将系统从\"一个 Python 回测工具\"提升为\"有行为金融学理论根基的量化决策系统\"——这正是 BA/FinTech 研究生项目最看重的思维深度。")}'
        '</div>',
        unsafe_allow_html=True,
    )
...
def _render_current_suggestion(config: dict):
    ...
    st.markdown("---")
    st.markdown(t("### 🌡️ 实时市场温度 · 当前建议"))

    with st.spinner(t("正在获取当前市场温度...")):
    ...
    with col1:
        ...
        if selected_idx and selected_idx.get("temperature"):
            ...
            label = temp.get("zone_label", t("适中"))
            ...
            st.markdown(
                ...
                f"<div style='font-size:0.85em;color:#888'>{idx_name} {t('当前温度')}</div>"
            ...
            st.info(t("{idx_name} 实时温度暂不可用").format(idx_name=idx_name))
    ...
    with col2:
        ...
        strategy_label = engine.STRATEGIES.get(strategy_name, {}).get("name", t("适中"))
        ...
        st.markdown(
            ...
            f"📐 {strategy_label}{t('型策略 · 当前决策')}</div>",
            unsafe_allow_html=True,
        )

        m1, m2, m3 = st.columns(3)
        with m1:
            st.metric(t("基准月投"), f"¥{base:,.0f}")
        with m2:
            st.metric(t("温度倍率"), f"×{multiplier}", delta=f"{multiplier - 1:.0%}" if multiplier != 1 else None)
        with m3:
            st.metric(t("建议月投"), f"¥{suggested:,.0f}",
        ...
        if saved > 0:
            st.markdown(
                ...
                f"{t('💰 本月存入现金池 <b>¥{saved:,.0f}</b>（温度偏高，暂扣）').format(saved=saved)}"
                f"<br><span style='color:#888;font-size:0.85em'>"
                f"{t('等温度回落至60°C以下，现金池自动释放加仓')}</span>"
                f"</div>",
                unsafe_allow_html=True,
            )
        elif multiplier > 1.0:
            ...
            st.markdown(
                ...
                f"{t('🚀 从现金池额外投入 <b>¥{extra:,.0f}</b>（温度偏低，加仓买入）').format(extra=extra)}"
                f"</div>",
                unsafe_allow_html=True,
            )
    ...
    if st.session_state.temp_dca_cash_pool > 0:
        st.markdown(
            ...
            f"📦 {t('累计现金池余额')}: ¥{st.session_state.temp_dca_cash_pool:,.0f}"
            f"</div>",
            unsafe_allow_html=True,
        )
...
def _render_footer(backtest: dict):
    ...
    st.markdown("---")
    st.caption(
        t("🌡️ 智能温度定投 — 磐策 PánCè 旗舰功能 · 回测区间: {start} 至 {end} · 共 {n} 个月 · 温度信号: {signal} · 数据源: {source} · 数据更新于 {ts} · 历史回测不代表未来收益，仅供参考").format(
            start=dq.get('date_start', '—'),
            end=dq.get('date_end', '—'),
            n=dq.get('total_months', 0),
            signal=t('PE估值(60%) + MA偏离(40%)') if dq.get('has_pe_data') else t('MA偏离(100%)'),
            source=dq.get('data_source', '—'),
            ts=ts,
        )
    )
    ...
    if pe_note:
        st.markdown("")
        st.info(t("📡 **数据覆盖说明**：{note}").format(note=pe_note))
...
def show():
    ...
    # Step 3: Run backtest
    with st.spinner(t("🔄 正在运行历史回测...（首次约 5-10 秒，后续秒出）")):
    ...
    if "error" in backtest:
        ...
        if "supported" in backtest:
            st.info(t("支持的指数: {indices}").format(indices=', '.join(backtest['supported'])))
        return
