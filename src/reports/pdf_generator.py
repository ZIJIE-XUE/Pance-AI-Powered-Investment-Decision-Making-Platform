...
class PDFGenerator:
    ...
    def __init__(self, output_path: str, lang: str = "zh"):
    ...
    def add_cover_page(self, user_info: dict):
        ...
        self.story.append(Paragraph(self._("智能投资顾问报告", "AI Investment Advisor Report"), self.styles["CoverTitle"]))
        ...
        self.story.append(Paragraph(
            self._(
                ...
                "This report is auto-generated by AI for educational reference only | "
                "Investing involves risk; exercise caution when entering the market.",
            ),
    ...
    def add_executive_summary(self, risk_profile: dict, portfolio: dict, simulation: dict):
        ...
        self.story.append(Paragraph(self._("报告摘要", "Executive Summary"), self.styles["SectionHeader"]))
        ...
        summary = (
            self._(
                ...
                f"Based on your risk assessment, your risk level is <b>{level}</b> (Score: {score:.0f}%). "
                f"We have configured a diversified portfolio with an expected annual return of <b>{ret:.2f}%</b>, "
                f"annual volatility of <b>{vol:.2f}%</b> (Sharpe Ratio: {sharpe:.2f}). "
                f"Monte Carlo simulation shows that over a {simulation.get('horizon_years', 'N/A')}-year investment horizon, "
                f"your portfolio has a <b>{prob:.1f}%</b> probability of positive returns, "
                f"with a median final value of <b>${median_final:,.0f}</b> (total return approx. {total_ret:.1f}%)."
            )
        )
    ...
    def add_risk_summary(self, risk_profile: dict):
        ...
        self.story.append(Paragraph(self._("1. 风险测评结果", "1. Risk Assessment Results"), self.styles["SectionHeader"]))
        ...
        self.story.append(Paragraph(
            self._(
                ...
                f"Risk Level: <b>{level}</b>&nbsp;&nbsp;|&nbsp;&nbsp;Risk Score: <b>{score:.0f}%</b>",
            ),
        ...
        if cat_scores:
            self.story.append(Paragraph(self._("各维度得分", "Dimension Scores"), self.styles["SubHeader"]))
            cat_labels = {
                "time_horizon": self._("投资期限适配度", "Investment Horizon Fit"),
                "financial_situation": self._("财务稳健度", "Financial Stability"),
                "risk_tolerance": self._("风险承受能力", "Risk Tolerance"),
                "investment_preference": self._("投资偏好进取度", "Investment Preference"),
                "knowledge_experience": self._("投资知识经验", "Knowledge & Experience"),
            }
    ...
    def add_portfolio_section(self, portfolio: dict, pie_chart_buf: io.BytesIO):
        ...
        self.story.append(Paragraph(self._("2. 投资组合配置", "2. Portfolio Allocation"), self.styles["SectionHeader"]))

        # Key metrics in a styled box
        self.story.append(Paragraph(self._("核心指标", "Key Metrics"), self.styles["SubHeader"]))
        metrics_text = self._(
            ...
            f"Expected Annual Return: <b>{portfolio['expected_return'] * 100:.2f}%</b>&nbsp;&nbsp;|&nbsp;&nbsp;"
            f"Annual Volatility: <b>{portfolio['volatility'] * 100:.2f}%</b>&nbsp;&nbsp;|&nbsp;&nbsp;"
            f"Sharpe Ratio: <b>{portfolio['sharpe_ratio']:.2f}</b>&nbsp;&nbsp;|&nbsp;&nbsp;"
            f"Est. Max Drawdown: <b>{portfolio['max_drawdown'] * 100:.2f}%</b>",
        )
        self.story.append(Paragraph(metrics_text, self.styles["HighlightBox"]))

        # Allocation table
        self.story.append(Paragraph(self._("持仓明细", "Holdings Detail"), self.styles["SubHeader"]))
        table_data = [[self._("代码", "Ticker"), self._("名称", "Name"), self._("资产类别", "Asset Class"), self._("配置比例", "Allocation"), self._("预期收益", "Expected Return"), self._("波动率", "Volatility")]]
        ...
        # Class summary
        class_map = {}
        class_labels = {"equity": self._("股票", "Equities"), "bond": self._("债券", "Bonds"), "gold": self._("黄金", "Gold"), "real_estate": self._("房地产", "Real Estate")}
        for a in portfolio["allocations"]:
            cls = a["asset_class"]
        ...
        self.story.append(Paragraph(
            self._("大类资产分布：", "Asset Class Distribution: ") + " &nbsp;|&nbsp; ".join(summary_parts),
            self.styles["BodyTextCN"],
        ))
    ...
    def add_simulation_section(self, simulation: dict, fan_chart_buf: io.BytesIO, hist_buf: io.BytesIO):
        ...
        self.story.append(Paragraph(self._("3. Monte Carlo 模拟分析", "3. Monte Carlo Simulation"), self.styles["SectionHeader"]))

        horizon = simulation.get("horizon_years", "N/A")
        initial = simulation.get("initial_amount", 0)
        paths = simulation.get("num_paths", 0)

        self.story.append(Paragraph(self._("模拟设定", "Simulation Settings"), self.styles["SubHeader"]))
        self.story.append(Paragraph(
            self._(
                f"初始投资 <b>¥{initial:,.0f}</b> | 投资期限 <b>{horizon}年</b> | "
                f"模拟 <b>{paths:,}</b> 条路径（基于几何布朗运动模型）",
                f"Initial Investment <b>${initial:,.0f}</b> | Horizon <b>{horizon} years</b> | "
                f"Simulated <b>{paths:,}</b> paths (based on Geometric Brownian Motion)",
            ),
            self.styles["BodyTextCN"],
        ))

        self.story.append(Paragraph(self._("关键统计", "Key Statistics"), self.styles["SubHeader"]))
        ...
        total_ret_pct = ((median / initial - 1) * 100) if initial else 0
        self.story.append(Paragraph(
            self._(
                f"• 预期终值（中位数）：<b>¥{median:,.0f}</b> | "
                f"总收益率约 <b>{total_ret_pct:.1f}%</b>",
                f"• Expected Final Value (Median): <b>${median:,.0f}</b> | "
                f"Total Return approx. <b>{total_ret_pct:.1f}%</b>",
            ),
            self.styles["BodyTextCN"],
        ))
        self.story.append(Paragraph(
            self._(
                ...
                f"• Pessimistic (P5, 95% lower bound): <b>${p5:,.0f}</b> | "
                f"Optimistic (P95, 95% upper bound): <b>${p95:,.0f}</b>",
            ),
            self.styles["BodyTextCN"],
        ))
        self.story.append(Paragraph(
            self._(
                ...
                f"• 95% VaR (Value at Risk): <b>${var95:,.0f}</b> | "
                f"Probability of Positive Return: <b>{prob:.1f}%</b>",
            ),
            self.styles["BodyTextCN"],
        ))
        ...
        self.story.append(Paragraph(self._("收益路径模拟图", "Return Path Simulation"), self.styles["SubHeader"]))
        fan_chart_buf.seek(0)
        self.story.append(Image(fan_chart_buf, width=160 * mm, height=100 * mm))

        # Histogram
        self.story.append(Paragraph(self._("终值概率分布", "Final Value Distribution"), self.styles["SubHeader"]))
        hist_buf.seek(0)
    ...
    def add_advisor_section(self, advisor_response: dict | None):
        ...
        self.story.append(Paragraph(self._("4. AI 投资顾问深度分析", "4. AI Advisor Analysis"), self.styles["SectionHeader"]))

        if advisor_response is None:
            self.story.append(Paragraph(
                self._(
                    "未生成AI分析。如需获取AI深度分析，请先在应用中点击「生成 AI 分析」按钮。",
                    "AI analysis has not been generated. To receive an in-depth AI analysis, "
                    "please click the \"Generate AI Analysis\" button in the application first.",
                ),
                self.styles["BodyTextCN"],
            ))
            return
        ...
        if s:
            self.story.append(Paragraph(self._("投资组合概述", "Portfolio Overview"), self.styles["SubHeader"]))
            self.story.append(Paragraph(s, self.styles["BodyTextCN"]))

        # Allocation rationale
        rationale = advisor_response.get("allocation_rationale", "")
        if rationale:
            self.story.append(Paragraph(self._("配置逻辑详解", "Allocation Rationale"), self.styles["SubHeader"]))
            self.story.append(Paragraph(rationale, self.styles["BodyTextCN"]))

        # Key risks
        risks = advisor_response.get("key_risks", [])
        if risks:
            self.story.append(Paragraph(self._("风险全景图", "Risk Landscape"), self.styles["SubHeader"]))
            for risk in risks:
        ...
        if scenarios:
            self.story.append(Paragraph(self._("市场情景推演", "Market Scenario Analysis"), self.styles["SubHeader"]))
            for sc in scenarios:
                self.story.append(Paragraph(
                    self._(
                        f"<b>{sc.get('scenario', '')}</b>（可能性：{sc.get('probability', 'N/A')}）",
                        f"<b>{sc.get('scenario', '')}</b> (Likelihood: {sc.get('probability', 'N/A')})",
                    ),
                    self.styles["BodyTextCN"],
                ))
                ...
                if sc.get("projected_impact"):
                    self.story.append(Paragraph(
                        self._(
                            ...
                            f"&nbsp;&nbsp;&nbsp;&nbsp;Projected Impact: {sc['projected_impact']}",
                        ),
                        self.styles["BodyTextCN"],
                    ))
        ...
        if recs:
            self.story.append(Paragraph(self._("投资建议", "Investment Recommendations"), self.styles["SubHeader"]))
    ...
    def add_financial_plan(self, user_info: dict, portfolio: dict, simulation: dict):
        ...
        self.story.append(Paragraph(self._("5. 个人财务规划建议", "5. Financial Planning"), self.styles["SectionHeader"]))

        age = user_info.get("age", 0)
        income = user_info.get("income", 0)
        assets = user_info.get("asset_size", 0)
        horizon = simulation.get("horizon_years", 5)
        _unspecified = self._("未指定", "Not specified")
        goal = user_info.get("investment_goal") or _unspecified
        ...
        if age < 30:
            advice_parts.append(
                self._(
                    ...
                    "You are in the early stage of wealth accumulation with a significant "
                    "time advantage. We recommend maintaining a high proportion of equity "
                    "allocation to leverage long-term compounding for asset growth. "
                    "Gradually increase regular investment amounts as your income grows.",
                )
            )
        elif age < 45:
            advice_parts.append(
                self._(
                    ...
                    "You are in the prime earning years of your career. Capitalize on income "
                    "growth to accelerate asset accumulation. We recommend a balanced "
                    "equity-bond allocation and starting systematic savings for children's "
            ...
            advice_parts.append(
                self._(
                    ...
                    "You have entered a phase where wealth preservation is as important as "
                    "growth. We recommend gradually reducing equity exposure, increasing "
                    "fixed-income assets, and focusing on cash flow stability and capital "
                    "preservation.",
                )
            )
        ...
        if income > 0:
            ...
            advice_parts.append(
                self._(
                    ...
                    f"Your investable assets are approximately {invest_ratio:.1f}x your "
                    f"annual income. We recommend allocating 15%-25% of monthly income "
                    f"to regular investments to build a disciplined investment habit.",
                )
            )

        # Goal-based
        if goal and goal != _unspecified:
            advice_parts.append(
                self._(
                    f"针对您的投资目标「{goal}」，我们建议将此投资组合作为核心配置，"
                    f"并根据目标的时间节点动态调整风险暴露。",
                    f"For your investment goal \"{goal}\", we recommend using this "
                    f"portfolio as your core allocation and dynamically adjusting "
                    f"risk exposure based on the goal's time horizon.",
                )
            )
        ...
        advice_parts.append(
            self._(
                ...
                f"Over the {horizon}-year investment horizon, we recommend rebalancing "
                f"the portfolio every six months to one year, restoring each asset class "
                f"to its target allocation to control risk and capture rebalancing returns.",
            )
        )
        ...
        # Action plan
        self.story.append(Paragraph(self._("行动建议", "Action Plan"), self.styles["SubHeader"]))
        steps = [
            self._(
                "立即行动：在完成风险评估后，按照推荐比例配置资产，避免因犹豫而错失市场机会。",
                "Act Now: After completing the risk assessment, allocate assets according "
                "to the recommended proportions to avoid missing market opportunities "
                "due to hesitation.",
            ),
            self._(
                "分批建仓：如有大额资金，建议分3-6个月逐步建仓，降低单点入市风险。",
                "Phased Entry: For large lump sums, consider building positions gradually "
                "over 3-6 months to reduce single-point entry risk.",
            ),
            self._(
                "定期定额：设定每月固定日期定额投资，利用平均成本法平滑市场波动。",
                "Dollar-Cost Averaging: Set up automatic monthly investments on a fixed "
                "date to smooth out market volatility through cost averaging.",
            ),
            self._(
                "年度检视：每年进行一次投资组合检视和风险重评，确保配置与人生阶段匹配。",
                "Annual Review: Conduct an annual portfolio review and risk reassessment "
                "to ensure your allocation matches your life stage.",
            ),
            self._(
                "持续学习：关注宏观经济和投资知识，提高独立判断能力，不盲从市场情绪。",
                "Continuous Learning: Stay informed on macroeconomics and investment "
                "knowledge, develop independent judgment, and avoid blindly following "
                "market sentiment.",
            ),
        ]
    ...
    def add_glossary(self):
        ...
        self.story.append(Paragraph(self._("6. 关键术语解释", "6. Key Terms"), self.styles["SectionHeader"]))
        terms = [
            (
                self._("夏普比率 (Sharpe Ratio)", "Sharpe Ratio"),
                self._(
                    "衡量投资组合每单位风险所获得的超额回报。大于1为良好，大于2为优秀。",
                    "Measures the excess return per unit of risk in a portfolio. "
                    "Above 1 is good; above 2 is excellent.",
                ),
            ),
            (
                self._("最大回撤 (Max Drawdown)", "Max Drawdown"),
                self._(
                    "投资组合从峰值到谷底的最大跌幅，反映最坏情况下的损失幅度。",
                    "The maximum decline from peak to trough in a portfolio, "
                    "reflecting the worst-case loss magnitude.",
                ),
            ),
            (
                self._("Monte Carlo 模拟", "Monte Carlo Simulation"),
                self._(
                    "基于随机过程的统计模拟方法，用于预测投资组合在未来不同市场条件下的可能表现。",
                    "A statistical simulation method based on stochastic processes, "
                    "used to project a portfolio's possible performance under "
                    "various future market conditions.",
                ),
            ),
            (
                self._("VaR (Value at Risk)", "VaR (Value at Risk)"),
                self._(
                    "在给定置信水平下，投资组合在特定时期内的最大可能损失。",
                    "The maximum potential loss of a portfolio over a specific "
                    "time period at a given confidence level.",
                ),
            ),
            (
                self._("几何布朗运动 (GBM)", "Geometric Brownian Motion (GBM)"),
                self._(
                    "描述资产价格随机波动的数学模型，是Monte Carlo模拟的基础。",
                    "A mathematical model describing the random movement of asset "
                    "prices, serving as the foundation for Monte Carlo simulation.",
                ),
            ),
            (
                self._("ETF (交易型开放式指数基金)", "ETF (Exchange-Traded Fund)"),
                self._(
                    "在交易所上市交易的指数基金，具有低成本、高透明度和流动性好的特点。",
                    "An index fund traded on exchanges, characterized by low cost, "
                    "high transparency, and good liquidity.",
                ),
            ),
        ]
    ...
    def add_disclaimer(self):
        """Add risk disclaimer."""
        self.story.append(Paragraph(
            self._("7. 风险提示与免责声明", "7. Risk Warnings & Disclaimer"),
        ...
        if disclaimer_path.exists():
            ...
            text = self._(
                ...
                "results. This report is for reference only and does not constitute "
                "investment advice.",
            )
